...
folder for reproducibility and code efficiency. In the code below, vector-format 
layers (points, etc.) are saved as GeoJSON files and raster-format layers are saved
as GeoTiffs. 
...
st_write(study_bounds, '../data/study_bounds.GeoJSON')
```
...
writeRaster(study_raster, '../data/study_raster.tif')
```
...
st_write(dv_bounds, '../data/entire_dv_bounds.GeoJSON', delete_layer=TRUE)

study_dv_bounds <- dv_bounds %>%
  st_intersection(study_bounds)

st_write(study_dv_bounds, '../data/study_dv_bounds.GeoJSON',delete_layer=TRUE)
```
...
writeRaster(dv_base, '../data/dv_iso_m.tif',overwrite=TRUE)
```
