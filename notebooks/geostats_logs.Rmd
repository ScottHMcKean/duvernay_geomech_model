...
# normal transform
rhob_norm = bestNormalize::orderNorm(shale_sp$rhob_mean)
shale_sf$rhob_norm = shale_sp$rhob_norm = rhob_norm$x.t
ggdensity(shale_sf, x='rhob_norm') +
  stat_overlay_normal_density(color='red', linetype='dashed')
moments::skewness(rhob_norm$x.t)
moments::kurtosis(rhob_norm$x.t)

# what is the global variance (should be 1...)
var(shale_sp$rhob_norm)

# make an experimental variogram
rhob_v = variogram(rhob_norm~1, shale_sp)
plot(rhob_v)

# check directional variograms
rhob_v_dir = variogram(rhob_norm~1, shale_sp, alpha=c(0,22,45,67,90,112,135,157))
plot(rhob_v_dir)
...
# make an experimental variogram
rhob_v = variogram(rhob_norm~1, carb_sp)
plot(rhob_v)
