...
This function retrieves all the log data and take quite a while. Don't run it..
We instead load the las_df from file (but can't provide this data sorry)
...
Read in the top picks and intersect the logs with each top and bottom for further
isolation and flattening across the study area.N ow aggregation of the logs 
...
```{r}
facies %>% merge(study_locs, by ='uwi') %>% st_write('../data/facies_data.GeoJSON')
```
