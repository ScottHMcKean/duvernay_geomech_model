...
```{r}
# secondary (SV)
sv_vgm = vgm(0, "Lin", 50E3)
sv_vgm = vgm(0, "Lin", 10E3, add.to=sv_vgm)
sv_vgm = vgm(0, "Lin", 5E3, add.to=sv_vgm)
g = gstat(id='sv', formula=svgrad_norm~1, data=sv_sp, beta=0, nmax=50)
g = gstat(g, id='sv', model=sv_vgm)

# primary (SH)
sh_vgm = vgm(0, "Lin", 50E3)
sh_vgm = vgm(0, "Lin", 10E3, add.to=sh_vgm)
sh_vgm = vgm(0, "Lin", 5E3, add.to=sh_vgm)
g = gstat(g, id='shmin', formula=shgrad_norm~1, data=sh_sp, beta=0, nmax=50)
g = gstat(g, id='shmin', model=sh_vgm)

# primary (PP)
pp_vgm = vgm(0, "Lin", 50E3)
pp_vgm = vgm(0, "Lin", 10E3, add.to=pp_vgm)
pp_vgm = vgm(0, "Lin", 5E3, add.to=pp_vgm)
g = gstat(g, id='pp', formula = ppgrad_norm~1, data=pp_sp, beta=0, nmax=50)
g = gstat(g, id='pp', model=pp_vgm)

# covariance
sv_sh_vgm = vgm(-0.5, "Lin", 50E3)
sv_sh_vgm = vgm(-0.5, "Lin", 10E3, add.to = sv_sh_vgm)
sv_sh_vgm = vgm(0, "Lin", 3E3, add.to=sv_sh_vgm)
g = gstat(g, id=c('sv','shmin'), model=sv_sh_vgm, beta=0, nmax=50)

sv_pp_vgm = vgm(-0.5, "Lin", 50E3)
sv_pp_vgm = vgm(-0.5, "Lin", 10E3, add.to = sv_pp_vgm)
sv_pp_vgm = vgm(0, "Lin", 3E3, add.to=sv_pp_vgm)
g = gstat(g, id=c('sv','pp'), model=sv_pp_vgm, beta=0, nmax=50)

sh_pp_vgm = vgm(-0.5, "Lin", 50E3)
sh_pp_vgm = vgm(0.5, "Lin", 10E3, add.to = sh_pp_vgm)
sh_pp_vgm = vgm(0, "Lin", 3E3, add.to=sh_pp_vgm)
g = gstat(g, id=c('shmin','pp'), model=sh_pp_vgm, beta=0, nmax=50)

v = variogram(g, cressie=TRUE)

g = fit.lmc(v,g, fit.lmc =TRUE)

plot(v,g)
g
```

Parse variogram for simulated annealing

```{r}
# get summary of variogram
...
vgm_sa = v %>%
  select(lag_dist = dist, npairs = np, variance = gamma)

vgm_export = list(vgm_stats, vgm_sa)
names(vgm_export) = c('vgm_stats', 'vgm')

saveRDS(vgm_export, 'stress_vgm_sa_export.rds')

```
We use omnidirectional variograms
