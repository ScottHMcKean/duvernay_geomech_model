...
# specify variogram
azimuth = vgm_stats %>% pull(azimuth)
dip = vgm_stats %>% pull(dip)
nlag = vgm_stats %>% pull(nlag)
tail = vgm_stats %>% pull(tail)
head = vgm_stats %>% pull(head)
vartype=1
...
#specify model
nugget = c(0, 5000, 10000, 50000, 0, 0, 0, 1)
spherical = c(1, 5000, 10000, 50000, 0, 0, 0, 1)
linear = c(5, 5000, 10000, 50000, 0, 0, 0, 1)
...
# define weighting_methods for optimization
if (weighting_method == 0){
  ...
  weighting_methods = gam[,2]/(1e-10+gam[,1])
}
