...
mldf_wres_num <- mldf_wres %>% 
  select("confining_pressure", "peak_stress_dev", 
    "residual_stress_dev", "youngs_modulus", "poisson_ratio", "bulk_density", 
    "brittleness", "pred_vp", "pred_vs") %>%
  rename(
...
mldf_nores_num <- mldf_nores %>% 
  select("confining_pressure", "peak_stress_dev", 
    "youngs_modulus", "poisson_ratio", "bulk_density", 
    "pred_vp", "pred_vs") %>%
  rename(
    "Confining Pressure (MPa)" = confining_pressure,
    "Peak Stress (MPa)" = peak_stress_dev, 
...
```{r}
# With Residuals
wres_feats = c("orientation", "group", "confining_pressure", "peak_stress_dev", 
"residual_stress_dev", "bulk_density", "pred_vp", "pred_vs")

mldf_wres_ym_norm = normalizeFeatures(
  mldf_wres %>% select(wres_feats, youngs_modulus),
...
mldf_wres_pr_norm = normalizeFeatures(
  mldf_wres %>% select(wres_feats, poisson_ratio),
...
mldf_wres_br_norm = normalizeFeatures(
  mldf_wres %>% select(wres_feats, brittleness),
  target ="brittleness")
...
# No residual
nores_feats = c("orientation", "group", "confining_pressure", "peak_stress_dev", 
"bulk_density", "pred_vp", "pred_vs")

mldf_nores_ym_norm = normalizeFeatures(
  mldf_nores %>% select(nores_feats, youngs_modulus),
  target ="youngs_modulus")

regr_fv_nores_ym = generateFilterValuesData(
  makeRegrTask(data = mldf_nores_ym_norm, target = "youngs_modulus"), 
  method = c("FSelector_information.gain")
  )
...
mldf_nores_pr_norm = normalizeFeatures(
  mldf_nores %>% select(nores_feats, poisson_ratio),
  target ="poisson_ratio")

regr_fv_nores_pr = generateFilterValuesData(
  makeRegrTask(data = mldf_nores_pr_norm, target = "poisson_ratio"), 
  method = c("FSelector_information.gain")
  )
...
ggsave("../output/mutual_information.pdf", grobs, width = 12, height = 6)
