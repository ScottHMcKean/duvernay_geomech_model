...
g = gstat(g, id='sh', formula=shgrad_norm~1, data=sh_sp, beta=0, nmax=50, model=vmodel)
g = gstat(g, id='pp', formula = ppgrad_norm~1, data=pp_sp, beta=0, nmax=50, model=vmodel)
g = gstat(g, id=c('sv','sh'), beta=0, nmax=50, model=vmodel)
g = gstat(g, id=c('sv','pp'), beta=0, nmax=50, model=vmodel)
g = gstat(g, id=c('sh','pp'), beta=0, nmax=50, model=vmodel)

# make variogram
v = variogram(g, cressie=TRUE)
```
Setup LMC simulated annealing problem and run the LMC algo
...
res <- fit_linear_model_of_coregionalization(
  ...
  weighting_method=6,maxiterations = 6000
  )
...
Save model when acceptable

```{r}
norm_objs = list(sv_norm, pp_norm, sh_norm)
names(norm_objs) = c('sv','pp','sh')

vgm_obj = list(v,g,lmc_in, norm_objs)
names(vgm_obj) = c('v','g','lmc_in','norm_objs')
saveRDS(vgm_obj, '20200508_stress_vgmobj.rds')
```

Debug LMC function

```{r}
azimuth = lmc_in$azimuth
dip = lmc_in$dip
nlag = lmc_in$nlag
tail = lmc_in$tail
head = lmc_in$head
